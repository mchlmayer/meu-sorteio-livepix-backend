...
app.get('/api/livepix/messages', async (req, res) => {
  ...
  try {
    // A API do LivePix não tem filtro de data direto para /messages,
    // então buscaremos um limite de mensagens e filtraremos no nosso backend.
    // Você pode ajustar o 'limit' se precisar de mais mensagens (máximo 100 na API do LivePix).
